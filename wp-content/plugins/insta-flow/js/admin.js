...
var FlowFlowApp = (function($){
  ...
  var Controller = {
    savedView : sessionStorage.getItem('grace_stream') || 'list',
    $body: null,
    $container : null,
    $sources : null,
    $list : null,
    ...
    editor: null,
    clip: null,
    activeTabIndex: parseInt(sessionStorage.getItem('grace_active_tab') || 0),
    ...
    restoreBackup: function (e) {
      ...
      promise.then(function success(){
        ...
        $.post(_ajaxurl, data).done(function(data){
          sessionStorage.setItem('grace_view_mode', 'list');
          sessionStorage.setItem('grace_active_tab', 0);
          location.reload();
        })
      }, function fail () {})
    },
    ...
    setupModelsAndViews : function () {
      ...
      var self = this;
      var savedScrollState = sessionStorage.getItem('grace_scroll');
      ...
      if ( this.savedView !== 'list' && streamRowModels.get(this.savedView) ) {
        ...
        streamRowModels.get(this.savedView).view.edit().then(function(id){
          ...
          setTimeout(function () {
            if (sessionStorage.getItem('grace-s' + id + '-tab') && streamModels.get(id)) {
              streamModels.get(id).view.$el.find('.view-tabs [data-tab="' + sessionStorage.getItem('grace-s' + id + '-tab') + '"]').trigger('click')
            }
          },0)
    ...
    attachGlobalEvents : function () {
      ...
      this.$container.find('.button-add').on('click', function(){
        ...
        if (!self.$container.find('#stream-view-new').length) {
          ...
          self.checkScrollWhenPreviewVisible(view.$el);
          view.saveViaAjax();
        }
      ...
      self.$tabs.on( 'click' , function() {
        ...
        sessionStorage.setItem('grace_active_tab', index);
      ...
      this.$form.delegate('.admin-button.submit-button', 'click', function (e) {
        ...
        function submitForm(opts) {
          ...
          sessionStorage.setItem('grace-section-submit', $t.attr('id'));
        }
      });
      ...
      this.$form.on('submit', function(e, opts){
        ...
        $.post(_ajaxurl, data, function( response ) {
          ...
          if( response == -1 ){
            ...
            $submitted = $('#' + sessionStorage.getItem('grace-section-submit'));
      ...
      $(window).unload(function (e) {
        sessionStorage.setItem('grace_scroll', $('body').scrollTop() || $('html').scrollTop());
      });
    ...
    backUrl: _ajaxurl + '?action=insta_flow_social_auth',
    ...
    switchToView : function (view) {
      ...
      sessionStorage.setItem('grace_stream', view);
    },

    checkScrollWhenPreviewVisible : function ($view) {
      var self = this;
      // remove all previous
      $(window).off('.stream');
      ...
      $(window).on('scroll.stream', eventData, self.handleScroll);
    },
    ...
    handleScroll : function (event) {
      ...
      if (!$preview.data('origTop')) {
        elTop = $preview.offset().top; // relative to document
        $preview.data('origTop', elTop);
      } else {
        elTop = $preview.data('origTop'); // relative to document
      }

      winTop = elTop - $(window).scrollTop();

      if (winTop < 50) {
        if (winTop > -450) {
          $preview.css('top', -winTop + 50)
        }
      } else {
        if (!flag) {
          $preview.css('top', '');
  ...
  StreamModel = Backbone.Model.extend({
    defaults: function () {
      return {
        ...
        "upic-style":            "round",
        "icon-style":            "label1",
        "cardcolor":             "rgb(62, 62, 62)",
        "namecolor":             "rgb(255, 255, 255)",
        "textcolor":             "rgb(241, 242, 243)",
        "linkscolor":            "rgb(89, 211, 243)",
        "restcolor":             "rgb(241, 239, 240)",
        "shadow":                "rgba(0, 0, 0, 0)",
        ...
        "c-s-smart-l":             "0",
        "c-s-smart-p":             "0",
        "c-rows":                "2", // todo remove
        "c-cols":                "5", // todo remove
        "c-arrows-always":       "yep",
        "s-desktop":             "15",
        "s-laptop":              "15",
        "s-tablet-l":            "10",
  ...
  StreamRowView = Backbone.View.extend({
    ...
    events: {
        "click [data-action='edit'], .td-name": "edit",
        "click [data-action='delete']": "destroy",
        "click [data-action='clone']": "clone",
    ...
    hideFeeds: function(){
        ...
        setTimeout(function () {
            ...
            var $feeds = _this.$('.highlight', $cell)
            var feedsWidth = 0
            var hiddenCount = 0
            ...
            $.each($feeds, function(i, feed){
                feedsWidth += feed.offsetWidth + 8 // margin
                $(feed).show()
    ...
    render: function(changed) {
      ...
      if (!this.rendered) {
        ...
        if (changed.hasOwnProperty('id')) {
          this.$el.find('.shortcode').html('[grace id="' + changed.id + '"]')
        }
    ...
    getFeedsStr: function (feeds) {
      ...
      feeds.forEach(function (feed, i, arr) {
          var type = feed['timeline-type'];
          var content = feed['content'];
          var icon;

          if(feed['location-meta'] && feed['location-meta']['name']){
          ...
          result += '<span class="highlight">' + icon + content + '</span>';
      })
    ...
    edit: function(e, viewStays) {
      ...
      var $view = Controller.$container.find('#stream-view-' + id);

      if (!$view.length) { // request data from server and add view
        ...
        request.done(
            function (response, status, xhr) {
              ...
              setTimeout(function () {
                ...
                Controller.checkScrollWhenPreviewVisible(view.$el);
              },100)
        ...
        if (!viewStays) Controller.switchToView(id);
        Controller.checkScrollWhenPreviewVisible($view);
        defer.resolve(id);
      }
  ...
  StreamView = Backbone.View.extend({
    ...
    events: {
      ...
      "click .shortcode": "selectShortcode",
      "change [id^=stream-layout]": "changeDesignMode",
      "change [id*=g-overlay]": "previewChangeOverlay",
      "change [id*=m-overlay]": "previewChangeOverlay",
      ...
      "change .design-step-2 input[id*=hidemeta]" : "previewChangeMeta",
    },
    ...
    initialize: function() {
      ...
      self.on('preview-update', function () {
        // Adds classes to container to display appropriate design
        var $item = self.$preview.find('.ff-item');
        var $first = $item.find('.ff-item-cont').children().first();
        var cls = $first[0].className;
        if (cls.indexOf('ff-item-meta') + 1) {
          $item.addClass('ff-meta-first');
        } else {
          $item.removeClass('ff-meta-first');
        }
        if (cls.indexOf('ff-img-holder ') + 1) {
          if ($first.next().is('.ff-item-meta')) {
            $item.addClass('ff-img-first');
          } else {
            $item.removeClass('ff-img-first');
          }
        } else {
          $item.removeClass('ff-img-first');
        }
    ...
    configDesign: function () {
      ...
      this.$el.find('[id^=stream-layout]:checked, select[id*=upic-pos], select[id*=upic-style], select[id*=icon-style], select[id*=icons-style], .design-step-2 select[id*=align], .design-step-2 input[id*=hidemeta]').change();
      // this.$el.find('.design-step-2 [id*=width]').keyup();

      // make preview sortable
      self.$el.find('.ff-item-cont').sortableCustom({
...
debugger
    ...
    applySavedTemplate: function () {
      ...
      $cont.find('.ff-overlay-wrapper').appendTo($cont);
    },
    ...
    renderConnectedFeeds: function () {
      ...
      for (var i = 0, len = feeds.length; i < len; i++) {
        ...
        if(feed.hasOwnProperty('location-meta') && feed['location-meta'].hasOwnProperty('name')){
            name = feed['location-meta']['name'];
        }
        ...
        items += '<span data-id="' +  feed.id +'" class="stream-feeds__item stream-feeds__' + feed.type +  (feed.errors && feed.errors.length ? ' stream-feeds__error' : '') + '">' + icon + stripslashes(name) + '</span>';
      }
    ...
    displayFeedsSelect: function () {
      ...
      for (i = 0, len = availableFeeds.length; i < len; i++) {
        ...
        options += '<option value="' + feed.id + '">' + name + '</option>';
      }
    ...
    changeDesignMode: function (e) {
      ...
      if(val == 'grid' || val == 'justified') {
        return false
      }

      this.previewChangeOverlay();
    ...
    previewChangeOverlay: function (e) {
      ...
      if( layout == 'justified' ||
          layout == 'carousel' ||
          (layout == 'grid' && this.model.get('g-overlay') == 'yep') ||
          (layout == 'masonry' && this.model.get('m-overlay') == 'yep'))
      {
        $preview.addClass('has-overlay');

      } else {
        $preview.removeClass('has-overlay');
      }
      // self.model.set('template', ['image', 'header', 'text', 'meta']);
    },
    ...
    previewChangeMeta: function(e){
      var $meta = this.$('.ff-item-meta');
      var checked = e.target.checked;
      if (checked) $meta.hide()
      else $meta.show();
    },
    ...
    saveViaAjax: function(e) {
      ...
      promise.done(function(serverModel){
        ...
        sessionStorage.setItem('grace_stream', serverModel.id);
  ...
  FeedsView = Backbone.View.extend({
    ...
    events: {
      ...
      "click .button-add": "createFeedView",
      "click .controls [data-action='toggle-dropdown']": "toggleDropDown",
      "mouseleave .controls": "popupLeave",
      "click [data-action='filter']": "filterFeed",
      "click [data-action='cache']": "resetFeedCache",
      "click [data-action='edit']": "editFeed",
      "click .highlight": "editFeed",
      "click [data-action='clone']": "cloneFeed",
      "click [data-action='delete']": "deleteFeed",
      ...
      "change .feed-view input[id$='type']": "updateTooltip",
      "change .feed-view textarea": "updateModel",
      "change .feed-view select": "updateModel",
      "change td .switcher": "toggleFeed",
      "mouseenter .feeds-list td.td-status": "showErrorIfPresent",
      "mouseleave .feeds-list td.td-status": "hideError",
    },
    ...
    render: function(){
      ...
      if (this.currentPage) {
        ...
        savedPage = parseInt(sessionStorage.getItem('grace_feeds_page')||1);
        if (savedPage > 1) {
          this.paginator.paginate(savedPage);
          this.currentPage = savedPage;
          sessionStorage.setItem('grace_feeds_page', -1); // one time
        }
      }
      ...
      renderFilters: function(uid){
          ...
          includeArr.forEach(function (item, i) {
              var $label =
                  '<li class="filter-label">' + item +
                  '<i data-action="delete-filter" data-type="exclude" data-content="' + item + '" class="flaticon-feed_type_user"></i>' +
                  '</li>'
    ...
    addFilter: function (e) {
        ...
        if(filters.indexOf(content) == -1){
            ...
            if(type == 'exclude'){
                ...
                this.model.attributes.feeds[id]['includes'] = filters.join(',')
            }
    ...
    savePage: function () {
      sessionStorage.setItem('grace_feeds_page', this.paginator._currentPageNum);
    },
    ...
    renderFeedsList: function () {
      ...
      if ( !_.isEmpty(feeds) ) {
        _.each(feeds, function (feed) {
          ...
          var type = settings['timeline-type'];

          if(content != ''){

            ival = stripslashes( content );

            if(feed.hasOwnProperty('location-meta') && feed['location-meta'].hasOwnProperty('name')){
              ival = feed['location-meta']['name']
            }
            ...
            info = info + '<span><span class="highlight">' + icon + ival + '</span></span>';

            if (settings.hasOwnProperty('mod')) info += '<span class="highlight grey">moderated</span>';
          }
        ...
        feedsListStr = '<tr><td  class="empty-cell" colspan="5">Please add at least one feed</td></tr>';
      }
    ...
    editFeed: function (e) {
      ...
      $view.addClass('feed-view-visible');
      ...
      this.updateTooltip($view, feed['timeline-type']);
    },
    ...
    createFeedView: function (e) {
      ...
      this.$popup.removeClass('add-feed-step-1').addClass('add-feed-step-2');

      // Init Instagram popup
      var $t = $( e.currentTarget );
      var $popup = this.$popup;
    ...
    updateTooltip: function ( flex, type ) {
      // Handle field hint visibility by Timeline type

      var $t = flex.currentTarget ? $( flex.currentTarget ) : flex;
      var val = type || $t.val();
      var $view = $t.closest('.feed-view');

      $view.find('.desc span').removeClass('visible');
      $view.find('.desc span[data-type="' + val + '"]').addClass('visible');
    },
    ...
    updateModel: function (event) {
      ...
      changed[id] = {'id': id, 'state': (isFresh ? 'created' : 'changed'), 'needReset' : !!$t.data('cache-reset')};
  ...
  var sectionExpandCollapse = (function($) {
    ...
    var storage = window.localStorage && JSON.parse(localStorage.getItem('grace_sections')) || {};
      ...
      initialize: function() {
        ...
        this.on('change', function(){
          if (window.localStorage) {
            ...
            window.localStorage.setItem('grace_sections', JSON.stringify(storage))
          }
        })
      },
      defaults : function () {
